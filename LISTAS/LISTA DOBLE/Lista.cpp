...
template <typename T>
ListaCircularDoble<T>::ListaCircularDoble() : primero(nullptr), ultimo(nullptr) {}
...
template <typename T>
Nodo<T>* ListaCircularDoble<T>::getPrimero() {
    return primero;
}

template <typename T>
Nodo<T>* ListaCircularDoble<T>::getUltimo() {
    return ultimo;
}
...
void ListaCircularDoble<T>::eliminarPorPlaca(string placa) {
    ...
    Nodo<T>* aux = primero;
    while (aux != nullptr && aux->getDato().getPlaca() != placa) {
        aux = aux->getSiguiente();
    }
